...
static inline void str2byte(unsigned char *dst, const unsigned char *src, uint32_t src_len)
{
	for (; src_len; src_len -= 2, src += 2)
		*dst++ = char2hex(src[0]) << 4 | char2hex(src[1]);
}
...
int main() {
  // const unsigned char key[16]={0x98,0xff,0xf6,0x7e,0x64,0xe4,0x6b,0xe5,0xee,0x2e,0x05,0xcc,0x9a,0xf6,0xd0,0x12};
	...
	char *keystr = "7fddb57453c241d03efbed3ac44e371c";
	unsigned char key[16];
	str2byte(key, keystr, strlen(keystr));
...
//   const unsigned char IV[12] ={0x2d,0xfb,0x42,0x9a,0x48,0x69,0x7c,0x34,0x00,0x6d,0xa8,0x86};
	...
	char *ivstr = "ee283a3fc75575e33efd4887";
	unsigned char IV[12];
	str2byte(IV, ivstr, strlen(ivstr));

	char *pstr = "d5de42b461646c255c87bd2962d3b9a2";
	unsigned char plaintext[3*16];
	str2byte(plaintext, pstr, strlen(pstr));
